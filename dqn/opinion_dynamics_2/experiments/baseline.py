...
def run_no_control_policy(env, max_steps=1000):
    ...
    for step in range(max_steps):
        ...
        if done or truncated:
            print(f"No-control policy finished after {step} steps. Done={done}, Truncated={truncated}")
            break
    ...
    return (
        ...
        np.array(all_intermediate_states) if len(all_intermediate_states) > 0 else np.array([])
    )